...
- Submit BUY orders only after: instrument lookup succeeded, strategy result explicitly returns BUY, and existing-order lookup shows there is no already completed/pending BUY exposure for the same tradingSymbol and exchange.
- Submit SELL/exit orders only after: instrument lookup succeeded, strategy result explicitly returns SELL, and existing-order lookup confirms an existing completed BUY/position for that tradingSymbol and exchange.
...
def system_prompt(config: AgentConfig) -> str:
    if not config.enable_order_tools:
        ...
        trading_policy = (
            ...
            "order tools only after strategy evidence returns an explicit BUY or SELL and existing-order "
            "inspection supports the action. Use the execution policy in the user prompt exactly. Never "
